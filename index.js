...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    const randomIndex = Math.floor(Math.random() * characters.length); // Визначаємо випадковий індекс.
    const randomChar = characters.charAt(randomIndex); // Отримуємо символ з випадковим індексом.
    password += randomChar; // Додаємо символ до паролю.
  }
...
function calculateCircleArea(radius) {
  ...
  const calculateCircle = Math.PI * radius ** 2;
...
function calculateHypotenuse(a, b) {
  ...
  const hypotenusC = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
  return hypotenusC;
...
function calculateVolumeCylinder(radius, height) {
  ...
  const calculateCylinder = Math.ceil(Math.PI * radius ** 2 * height);
...
function getFractionalPart(num) {
  ...
  const getFractional = Math.trunc(num);
  const part = getFractional - num;
  const fractional = Math.fround(part);
  return fractional;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sum = Math.round(principal * (1 + interestRate) ** years);
  return sum;
